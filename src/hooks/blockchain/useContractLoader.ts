...
export interface ContractLoaderConfig {
  ...
  chainId?: string
  hardhatContracts?: unknown
  externalContracts?: unknown
  hardhatNetworkName?: string
}

export default function useContractLoader(
  providerOrSigner,
  config: ContractLoaderConfig = {}
) {
  const [contracts, setContracts] = useState({})

  const customAddressKeys =
    config.customAddresses && Object.keys(config.customAddresses).join()
  const customAddressValues =
    config.customAddresses && Object.values(config.customAddresses).join()
